...
class MemoryEngine:
    ...
    async def _scroll_all(self, collection: str) -> dict[str, tuple[list[float], dict]]:
        points: dict[str, tuple[list[float], dict]] = {}
        offset = None
        while True:
            ...
            if not batch:
                break
            for pt in batch:
                points[str(pt.id)] = (pt.vector, pt.payload)
            if offset is None:
                break
        return points

    async def migrate_collection_prefix(self, old_prefix: str) -> int:
        new_prefix = f"{self._config.collection_prefix}_"
        old_prefix_full = f"{old_prefix}_"
        collections = (await self._qdrant.get_collections()).collections
        migrated = 0

        for col in collections:
            if not col.name.startswith(old_prefix_full):
                continue

            slug = col.name[len(old_prefix_full):]
            new_name = f"{new_prefix}{slug}"

            old_info = await self._qdrant.get_collection(col.name)
            if not old_info.points_count:
                await self._qdrant.delete_collection(col.name)
                logger.info("Deleted empty old collection %s", col.name)
                continue

            existing_names = {c.name for c in collections}

            if new_name in existing_names:
                await self._qdrant.delete_collection(new_name)
                logger.info("Removing previous migration attempt %s — starting fresh", new_name)
            ...
            try:
                old_points = await self._scroll_all(col.name)

                for point_id, (vector, payload) in old_points.items():
                    await self._qdrant.upsert(
                        collection_name=new_name,
                        points=[PointStruct(id=point_id, vector=vector, payload=payload)],
                    )

                new_points = await self._scroll_all(new_name)

                mismatches = []
                for point_id, (old_vec, old_pay) in old_points.items():
                    if point_id not in new_points:
                        mismatches.append(f"  {point_id}: missing from new collection")
                        continue
                    new_vec, new_pay = new_points[point_id]
                    if old_pay != new_pay:
                        mismatches.append(f"  {point_id}: payload mismatch")
                    if old_vec != new_vec:
                        mismatches.append(f"  {point_id}: vector mismatch")

                if mismatches:
                    detail = "\n".join(mismatches)
                    logger.error(
                        "Migration verification FAILED for %s — keeping both collections:\n%s",
                        col.name, detail,
                    )
                    continue

                logger.info(
                    "Migration verified for %s → %s: %d points, all IDs/payloads/vectors match",
                    col.name, new_name, len(old_points),
                )
                await self._qdrant.delete_collection(col.name)
                migrated += 1
                logger.info("Removed old collection %s", col.name)

            except Exception:
                logger.exception(
                    "Migration failed for %s — cleaning up partial copy %s",
                    col.name, new_name,
                )
                try:
                    await self._qdrant.delete_collection(new_name)
